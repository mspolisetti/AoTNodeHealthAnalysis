...
sensor.list.main = read.csv(  "sensors.csv" , stringsAsFactors = FALSE)
   ...
   aggrData = data %>% 
    ...
    dplyr::summarize(
     ...
     , sensorStatus = case_when(
      (max(value_hrf) - min(value_hrf) == 0) & (sum(withinBounds == FALSE) > 0) ~ 3, 
      (max(value_hrf) - min(value_hrf) == 0) ~ 1, 
      (sum(withinBounds == FALSE) > 0) ~ 2, 
      TRUE ~ 0)) 
...
createVis = function(df, nodeid) {
  ...
  #the ggplot
      ...
      #tile layer
      geom_tile(colour="white",size=0.5,aes(fill=factor(value))) +
      coord_fixed() +
      labs(x="",y="")+
      #setting the color
 ...
 ggsave(filename="heatmap.png",plot = p,dpi=150,type="cairo")
...
```{r}
createNodeFindings = function(node.1, nodeid) {
  inputfilename = paste("node_" , nodeid , ".csv", sep = "")
  datapath2 = "Node-outputs"
  node.1 = read.csv(paste(datapath2, inputfilename, sep = "/"), stringsAsFactors = FALSE)
...
```{r}
createNodeFindings(node.1, "001e0610ee33")
